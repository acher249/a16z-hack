...
function createStripe(platformFunctions, requestSender = defaultRequestSenderFactory) {
  Stripe2.PACKAGE_VERSION = "16.4.0";
...
if (env.SST_KEY_FILE && env.SST_KEY && !globalThis.SST_KEY_FILE_DATA) {
  ...
  const decipher = crypto4.createDecipheriv("aes-256-gcm", key, nonce);
...
var Billing2;
((Billing3) => {
  function compute(storage) {
    const rate = storage <= 10 ? 4 : storage <= 100 ? 2 : 1;
    return rate * storage * 100;
  }
  Billing3.compute = compute;
  __name(compute, "compute");
})(Billing2 || (Billing2 = {}));

// packages/functions/src/billing.ts
var main = Util.handler(async (event) => {
  const { storage, source } = JSON.parse(event.body || "{}");
  const amount = Billing2.compute(storage);
  const description = "Scratch charge";
  const stripe = new stripe_esm_node_default(
    // Load our secret key
